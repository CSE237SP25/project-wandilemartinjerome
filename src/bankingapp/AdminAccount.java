...
public class AdminAccount extends AccountHolder {
     ...
     * 
     * @param accountDatabase The system's account database
     * @param accountNumber The account number to freeze
     * @return true if successful, false otherwise
     */
    public boolean freezeCustomerAccount(BankAccountDatabase accountDatabase, int accountNumber) {
        if (!isActive) {
            return false;
        }
        
        boolean result = accountDatabase.freezeAccount(accountNumber);
        if (result) {
            logAction("Froze account with number: " + accountNumber);
        } else {
            logAction("Failed to freeze account with number: " + accountNumber);
        }
        return result;
    }
     ...
     * @return true if successful, false otherwise
     */
    public boolean unfreezeCustomerAccount(BankAccountDatabase accountDatabase, int accountNumber) {
        if (!isActive) {
            return false;
        }
        
        boolean result = accountDatabase.unfreezeAccount(accountNumber);
        if (result) {
            logAction("Unfroze account with number: " + accountNumber);
        } else {
            logAction("Failed to unfreeze account with number: " + accountNumber);
        }
        return result;
    }
     ...
     */
    public boolean isCustomerAccountActive(BankAccountDatabase accountDatabase, int accountNumber) {
        if (!isActive) {
            return false;
        }
        
        return accountDatabase.isAccountActive(accountNumber);
    }
     ...
     */
    public String generateAccountStatusReport(BankAccountDatabase accountDatabase) {
        if (!isActive) {
            return "Admin account inactive";
        }
        ...
        report.append("Total accounts: ").append(accountDatabase.getAccountCount()).append("\n");
        report.append("Active accounts: ").append(accountDatabase.getActiveAccountCount()).append("\n");
        report.append("Frozen accounts: ").append(accountDatabase.getFrozenAccountCount()).append("\n");
